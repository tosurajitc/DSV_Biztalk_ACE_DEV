...
def render_vector_db_controls():
    ...
    if st.session_state.vector_ready:
        ...
        st.info("Upload PDF in Agent 1 to initialize")
...
def render_program_1_ui():
    """Render Program 1: Specification-Driven BizTalk ACE Mapper UI"""
    st.header("Agent 1: Specification-Driven BizTalk to ACE Mapper")
    st.markdown("**Business specification-driven intelligent mapping** - Optimized for token efficiency and mapping quality")
    ...
    with col1:
        st.markdown("**1. BizTalk Project Folder**")
        biztalk_folder = st.text_input(
            "BizTalk Project Root Folder",
            value=r"C:\@Official\@Gen AI\DSV\BizTalk\MH.ESB.EE.Out.DocPackApp\MH.ESB.EE.Out.DocPackApp",
            help="Root folder containing BizTalk project files (will scan 3-4 levels deep)"
        )

            

        if biztalk_folder and os.path.exists(biztalk_folder):
            st.session_state['biztalk_folder'] = biztalk_folder
            # Quick preview of what will be scanned
            ...
            for root, dirs, files in os.walk(biztalk_folder):
                for file in files:
                    if Path(file).suffix.lower() in extensions:
                        ...
                        if len(found_files) >= 10:  # Limit preview
                            break
                if len(found_files) >= 10:
                    break
            
            st.success(f"  Folder valid - Found {len(found_files)}+ BizTalk files")
            with st.expander("  Sample Files Found"):
                for file in found_files[:8]:
                    file_icon = {
                        '.btproj': '  ', '.odx': '', '.btm': '', 
                        '.xsl': '', '.xsd': '', '.cs': ''
                    }.get(Path(file).suffix.lower(), '')
                    st.text(f"{file_icon} {file}")
        elif biztalk_folder:
            st.error("  Folder path does not exist")
    
    with col2:
        st.markdown("**2. Business Specification (Optional)**")
        confluence_pdf = st.file_uploader(
            "Upload Confluence/Business Requirements PDF to create the Vector DB",
            type=['pdf'],
            help="Business specification document for intelligent mapping"
        )
        ...
        if confluence_pdf:
                ...
                # VECTOR DB SETUP SECTION (NEW)
                if VECTOR_DB_AVAILABLE and st.session_state.vector_enabled:
                    st.subheader("  Vector Knowledge Base Setup")
                    
                    if not st.session_state.vector_ready:
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("  Setup Vector Knowledge Base", type="primary"):
                                success = setup_vector_knowledge_base(confluence_pdf, "Agent 1")
                                if success: 
                                    st.rerun()
                        with col2:
                            st.info("Setup Vector DB to optimize all agents")
                    else:
    ...
    with col1:
        groq_api_key = st.text_input(
            ...
            type="password",
            help="Required for intelligent mapping analysis"
        )
        
        groq_model = st.selectbox(
            "LLM Model",
            ["llama-3.3-70b-versatile", "llama-3.1-8b-instant"],
            help="Choose AI model for specification analysis"
        )
    ...
    with col2:
        output_dir = st.text_input(
            "Output Directory",
            value="output",
            help="Directory for generated mapping files"
        )
        
        # Show what will be generated
        st.info("  **Generated Files:**\n- biztalk_ace_component_mapping.xlsx\n- ESQL_Template_Updated.esql\n- biztalk_ace_component_mapping.json")

    # Validation
    missing_inputs = []
    if not biztalk_folder or not os.path.exists(biztalk_folder):
        missing_inputs.append("Valid BizTalk folder path")
    if not groq_api_key:
    ...
    # Execute Button with Vector DB Status Check
    vector_ready = st.session_state.get('vector_ready', False)
    vector_enabled = st.session_state.get('vector_enabled', False)
    pdf_processed = st.session_state.get('confluence_pdf_processed', False)

    if vector_ready:
        #   Vector DB ready - Active button
        if st.button("  Run Agent 1", type="primary", key="run_spec_mapping"):
            run_specification_mapping(biztalk_folder, confluence_pdf, groq_api_key, groq_model, output_dir)
    else:
        #   Vector DB not ready - Disabled button with guidance
        st.button("  Run Agent 1", type="primary", disabled=True, key="run_spec_mapping_disabled")
        
        # Show specific guidance based on current state
        if not vector_enabled:
            st.error("  **Button Locked**: Vector DB is disabled")
            st.info("  **Next Step**: Enable Vector DB in the sidebar")
        elif not pdf_processed:
            st.error("  **Button Locked**: No business specification uploaded")
            st.info("  **Next Step**: Upload your business specification PDF above")
        elif pdf_processed and not vector_ready:
            st.error("  **Button Locked**: Vector Knowledge Base not set up")
            st.info("  **Next Step**: Click '  Setup Vector Knowledge Base' button above")
        else:
            st.error("  **Button Locked**: Vector DB not ready")
            st.info("  **Next Step**: Complete Vector Knowledge Base setup")
...
def render_program_2_ui():
    ...
    st.markdown("**Enhanced**: Generate enterprise-ready DSV standard MessageFlow with Vector DB optimization")
    ...
    st.markdown("---")
    st.subheader("DSV MessageFlow Configuration")
    ...
    with col1:
        app_name = st.text_input(
            ...
            value="EPIS_CW1_Document_App",
            help="Name for the generated DSV ACE application"
        )
        
        flow_name = st.text_input(
            "MessageFlow Name", 
            value="CW1_IN_Document_SND",
...
def render_program_3_ui():
    ...
    st.markdown("**Enhanced BizTalk to ACE Migration** - Pure AI-driven generation with orchestrated modules manages 5 Agents" \
...
def render_program_4_ui():
    ...
    st.markdown("Template-driven quality analysis with <10K token efficiency")
    ...
    if program_3_status != 'success' or not program_3_output:
        st.warning("**Program 4 Needs**: The ACE components from Program 3 for quality check")
        return
...
def render_program_5_ui():
    ...
    if program_4_status != 'success' or not program_4_output:
        st.warning("   **Program 4 Required**: Complete Migration Quality Reviewer first to generate reviewed ACE components")
...
def render_token_analytics_tab():
    ...
    with st.expander("   Advanced Capacity Planning Calculator", expanded=False):
        ...
        with col1:
            st.markdown("**ðŸ“ Flow Parameters**")
